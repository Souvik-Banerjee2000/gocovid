...
class UserProfileResource(Resource):
    ...
    def post(self):
        ...
        user = UserProfile(
            username=data['username'], password=data['password'], isadmin=data['isadmin'], no_of_posts=0)
        user.save_to_db()
...
class AddPosts(Resource):
    ...
    def post(self):
        ...
        post = UserPosts(user_name=current_user.username, home_town=data['home_town'], OutingNo=int(
            data['OutingNo']), RatioOut=int(data['RatioOut']), RatioIn=int(data['RatioIn']), user=current_user)
        post.save_to_db()
